...
import { listDrugs, createDrug } from '../lib/store.js';
...
type FhirResource = FhirMedicationRequest | FhirMedicationStatement | FhirMedicationAdministration;
...
type ExtendedDrugRecord = DrugRecord & {
  rxNormCode?: string;
  roAliases?: string[];
};
...
function extractMedicationName(concept?: FhirCodeableConcept, display?: string): string {
...
function extractDose(dosages?: FhirDosage[]): { amount: number | null; unit: string | null } {
...
function extractRoute(dosages?: FhirDosage[]): string | null {
  ...
  const text = route.text ?? route.coding?.[0]?.display ?? route.coding?.[0]?.code ?? null;
  if (!text) return null;
  ...
  for (const entry of bundle.entry ?? []) {
    ...
    if (resource.resourceType === 'MedicationRequest') {
      ...
      results.push({
        ...
        unit,
        route,
        startDate: r.authoredOn?.slice(0, 10) ?? r.dispenseRequest?.validityPeriod?.start?.slice(0, 10) ?? null,
      ...
      results.push({
        ...
        unit,
        route,
        startDate: r.effectivePeriod?.start?.slice(0, 10) ?? r.effectiveDateTime?.slice(0, 10) ?? null,
      ...
      const routeText = r.dosage?.route?.text ?? r.dosage?.route?.coding?.[0]?.display ?? null;
      results.push({
        name,
        rxNormCode,
...
function matchMedication(parsed: ParsedMedication, drugs: ExtendedDrugRecord[]): ExtendedDrugRecord | null {
  ...
  for (const drug of drugs) {
    if (drug.name.toLowerCase().includes(nameLower) || nameLower.includes(drug.name.toLowerCase())) {
      return drug;
    }
    if (drug.genericName && (drug.genericName.toLowerCase().includes(nameLower) || nameLower.includes(drug.genericName.toLowerCase()))) {
      return drug;
    }
    const aliases: string[] = drug.roAliases ?? [];
    if (aliases.some(alias => alias.toLowerCase() === nameLower || nameLower.includes(alias.toLowerCase()))) {
      return drug;
    }
  }
...
async function lookupRxNorm(name: string, rxNormCode: string | null): Promise<Partial<DrugRecord> | null> {
  try {
    ...
    const getprop = (propName: string) => props.find(p => p.propName === propName)?.propValue ?? null;
 ...
 * /fhir/import:
 *   post:
 *     summary: Import medications from a FHIR Bundle
 *     description: |
 *       Accepts a FHIR R4 Bundle containing MedicationRequest, MedicationStatement, and/or
 *       MedicationAdministration resources. Matches medications against the local drug library
 *       by name and RxNorm code. Unmatched drugs are looked up via the public RxNorm API
 *       and auto-created in the library.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             description: A FHIR R4 Bundle resource
 *     responses:
 *       200:
 *         description: Resolved medication list
 *       400:
...
router.post('/fhir/import', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!bundle || bundle.resourceType !== 'Bundle') {
      res.status(400).json({ error: 'Request body must be a FHIR Bundle resource.' });
      return;
    }
    ...
    if (!parsed.length) {
      res.json({
        ...
        totalMatched: 0,
        totalUnmatched: 0,
        message: 'No MedicationRequest, MedicationStatement, or MedicationAdministration resources found in the bundle.',
      });
      return;
    }

    const drugs = (await listDrugs()) as ExtendedDrugRecord[];
    ...
    for (const med of parsed) {
      ...
      if (drug) {
        ...
        log.info({ name: med.name, rxNormCode: med.rxNormCode }, 'Unmatched FHIR medication — looking up via RxNorm');
        ...
        if (rxInfo) {
          try {
            ...
            drugs.push(created as ExtendedDrugRecord);
            autoCreated.push(created);
            matchResults.push({ parsed: med, drug: created, isNew: true });
          } catch (createErr) {
            log.warn({ err: createErr, name: med.name }, 'Failed to auto-create drug from RxNorm lookup');
    ...
    const matched = matchResults.filter(r => r.drug !== null).map(r => ({
      parsedMedication: r.parsed,
      drug: r.drug,
      isNewDrug: r.isNew,
    }));

    const unmatched = matchResults.filter(r => r.drug === null).map(r => r.parsed);
    ...
    log.error({ err }, 'FHIR import failed');
