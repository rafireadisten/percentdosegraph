...
function App() {
  ...
  function handleFhirImportConfirm({ matchedDrugs, matchedMedicationEntries }) {
    setDrugs(current => mergeDrugCatalog(current, matchedDrugs));
    setMedicationEntries(current => mergeMedicationEntries(current, matchedMedicationEntries));
    setSelectedDrugIds(current => {
      const merged = new Set(current);
...
function FhirImportModal({ drugs, onClose, onConfirm }) {
  ...
  const [unmatchedOverrides, setUnmatchedOverrides] = React.useState({});
  ...
  function handleConfirm() {
    ...
    for (const match of selectedMatches) {
      ...
      const entry = normalizeMedicationEntry({
        ...
        notes: parsed.dose != null
          ? `Imported from FHIR (${parsed.sourceResourceType}). Dose: ${parsed.dose} ${parsed.unit ?? ''}.`.trim()
          : `Imported from FHIR (${parsed.sourceResourceType}).`,
        createdAt: new Date().toISOString(),
      });
  ...
  const checkedCount = checkedIds.size + Object.keys(unmatchedOverrides).filter(k => unmatchedOverrides[k]).length;
    ...
    h(
      ...
      !importResult
        ...
        : h(
            ...
            importResult.autoCreated?.length > 0
              ? h(
                  'p',
            ...
            importResult.matched.length > 0
              ? h(
                  ...
                  h(
                    ...
                    importResult.matched.map((match, i) =>
                      h(
                        'label',
                        ...
                        h(
                          ...
                          match.parsedMedication.name !== match.drug.name
                            ? h('span', { className: 'fhir-match-original' }, ` (from "${match.parsedMedication.name}")`)
                            : null,
            ...
            importResult.unmatched.length > 0
              ? h(
                  ...
                  h(
                    ...
                    importResult.unmatched.map((parsed, i) =>
                      h(
                        'div',
